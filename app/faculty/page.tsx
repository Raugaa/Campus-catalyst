...
export default function FacultyDashboard() {
  ...
  // Department Data for Donut Chart (simulated)
  ...
  // Simulated data for different years
  ...
  // Function to create a more advanced pie chart
  const renderEnhancedPieChart = () => {
    const total = currentData.department.reduce((sum: number, dept: any) => sum + dept.value, 0);
      ...
      <div className="flex flex-col lg:flex-row items-center justify-center gap-8">
        <div className="relative w-64 h-64">
          ...
          <svg viewBox="0 0 100 100" className="w-full h-full">
            {currentData.department.map((dept: any, index: number) => {
  ...
  // Function to create an enhanced bar graph
  const renderEnhancedBarGraph = () => {
    // Find the maximum value for scaling
    const maxValue = Math.max(
      ...currentData.weeklyActivity.map((data: any) => Math.max(data.activity, data.engagement))
    );
        ...
        <div className="h-64 flex items-end space-x-4 relative pt-6">
          ...
          <div className="flex-1 ml-8 relative">
            ...
            <div className="h-full flex items-end space-x-6 pt-4">
              {currentData.weeklyActivity.map((data: any, index: number) => (
